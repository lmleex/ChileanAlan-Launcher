...
async function setBackground(theme) {
    ...
    if (fs.existsSync(`${__dirname}/assets/images/background/easterEgg`) && Math.random() < 0.005) {
        ...
        backgroundPath = backgrounds[Math.floor(Math.random() * backgrounds.length)];
        backgroundPath = `./assets/images/background/easterEgg/${backgroundPath}`;
        ...
        backgroundPath = backgrounds[Math.floor(Math.random() * backgrounds.length)];
        backgroundPath = `./assets/images/background/${theme ? 'dark' : 'light'}/${backgroundPath}`;
    }

    // Check if it's a video
    if (backgroundPath && /\.(mp4)$/.test(backgroundPath)) {
        isVideo = true;
    }
    ...
    if (isVideo) {
        ...
        videoElement.src = backgroundPath;
        videoElement.style.display = 'block';

        // Hide background image if video is playing
        body.style.backgroundImage = 'none';
    } else {
        // Hide video if an image is being used
        let videoElement = document.getElementById('backgroundVideo');
        if (videoElement) {
            videoElement.style.display = 'none';
        }

        body.style.backgroundImage = backgroundPath ? `linear-gradient(#00000080, #00000080), url(${backgroundPath})` : (theme ? '#000' : '#fff');
