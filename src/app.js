...
ipcMain.on('new-status-discord', async (event, instancia, imginstancia) => {
    client.login({ clientId: '1243071015219892234' });
    client.on('ready', () => {
        client.request('SET_ACTIVITY', {
            pid: process.pid,
            activity: {
                details: 'Iniciando Una Aventura!',
                assets: {
                    large_image: 'chiletiana',
                    large_text: `Twitch.tv/Chiletian_CL`,
...
ipcMain.on('new-status-discord-jugando', async (event, instancia, imginstancia, version) => {
    ...
    client.on('ready', () => {
        client.request('SET_ACTIVITY', {
            ...
            activity: {
                details: `${instancia}`,
                state: `ChiletianStudios`,
                assets: {
                    large_image: 'chiletiana',
                    large_text: `Twitch.tv/alan2k2tv`,
                    small_image: 'steve',
                    small_text: `¡Entra!`
                },
                instance: false,
